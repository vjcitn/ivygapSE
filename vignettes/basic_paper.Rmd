...
fit_mixed_logistic_CT = function( SE = splim, gene, dataframe, verbose=FALSE ) {

  stopifnot(all(c("id", "bl", "isCTH") %in% names(dataframe)))
  stopifnot(gene %in% rownames(SE))
  dataframe[[gene]] = as.numeric(log(assay(SE[gene,])+1))
  fmla = as.formula(paste("isCTH", "~", gene))
  glmmPQL(fmla, data=dataframe, fam=binomial, random=~1|id/bl, verbose=verbose)

  }

ff_CT = lapply(useful, function(x) fit_mixed_logistic_CT( SE=splim, gene=x, dataframe=newdf) )
names(ff_CT) = useful
summary(ff_CT[[1]])
...
# }
# 
# ff_CT_two_genes = lapply(useful, function(x) fit_mixed_logistic_CT_two_genes(SE=splim, gene=x, dataframe=newdf) )
# names(ff_CT_two_genes) = useful
# summary(ff_CT_two_genes[[1]])
