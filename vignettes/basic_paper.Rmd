...
fit_mixed_logistic = function( SE = splim, gene, dataframe ) {
  stopifnot(all(c("id", "bl", "isLE") %in% names(dataframe)))
  stopifnot(gene %in% rownames(SE))
  newdf[[gene]] = as.numeric(log(assay(SE[gene,])+1)
  fmla = paste("isLE", "~", gene)
  glmmPQL(fmla, data=dataframe, fam=binomial, random=~1|id/bl)
  }
